...
void RecursivePack(CEterPack & pack, const char * filename, std::vector<std::string>& vecCompressedTextures, std::vector<int>& vecCompressedTexMipMaps)
{
    ...
    if ((hFind = FindFirstFile(filename, &fdata)) != INVALID_HANDLE_VALUE)
    {
		do
		{
			...
			if ((p = strrchr(temp, '*'))!=NULL)	// µÚ¿¡ * À» ¾ø¾Ø´Ù.
				*p = '\0';
...
int main(int argc, char **argv)
{
	...
	if (!strcmp(argv[1], "--createiv"))
	{
		...
		if (diskFile.Create(argv[2], CFileBase::FILEMODE_WRITE))
		{
			...
			_chmod(argv[2], _S_IREAD); // ÀĞ±â Àü¿ëÀ¸·Î ¸¸µé±â
			return 0;
		}
	...
	// ExcludedFolderNameList - ÇØ´ç Æú´õ ÀÌ¸§Àº ÆÑÇÏÁö ¾Ê´Â´Ù.
	...
	// ExcludedPathList - ÆĞ½º ³»¿¡ ¸ğµç °ÍÀ» ÆÑÇÏÁö ¾Ê´Â´Ù.
	...
	// ExcludedFileNameList - ¸ğµç Æú´õ¿¡ ´ëÇØ ÇØ´ç ÆÄÀÏÀÌ¸§Àº ÆÑÇÏÁö ¾Ê´Â´Ù.
	...
	// OldCompressExtNameList - ÇØ´ç È®ÀåÀÚ¸¦ °¡Áø ÆÄÀÏÀº ¾ÏÈ£È­ ÇÑ´Ù.
	if (TextFileLoader.GetTokenVector("oldcompressextnamelist", &pTokenVector))
	{
		...
		for (; pTokenVector->end() != itor; ++itor)
		{
			...
			// OldCompressExtNameList´Â ¾ĞÃàÀ» ÇÏ´Â °ÍÀÌ ¾Æ´Ï¶ó ¾ÏÈ£È­¸¦ ÇØ¾ßÇÏ¹Ç·Î
			// ÀÇµµÀûÀ¸·Î COMPRESSED_TYPE_SECURITY¸¦ ³Ö¾úÀ½
			RegisterPackTypeByExtName(rstrName.c_str(), COMPRESSED_TYPE_SECURITY);
		}
	}
	...
	// filelist¿¡ ¼½¼Ç¿¡ ÀÔ·ÂµÇ¾î ÀÖ´Â ¸ğµç ÆÄÀÏÀ» ÆÑÇÏ±â
